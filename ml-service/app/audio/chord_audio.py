...
class ChordAudioAnalyzer:
    ...
    def analyze_base64_audio(
        ...
        if not audio_base64:
            return self._empty("No audio clip was provided.")
        ...
        if normalized_format not in supported_formats:
            return self._empty(
                "Unsupported audio format. Send WAV, signed 16-bit PCM, or float32 PCM."
            )

        try:
            if normalized_format in {"pcm_s16le", "pcm_f32le"}:
                samples = self._decode_pcm(audio_base64, normalized_format, channels)
            else:
        ...
        if samples.size == 0:
            return self._empty("Audio clip was empty.")

        rms = float(np.sqrt(np.mean(samples ** 2)))
        if rms < self.min_rms:
            return self._empty("Audio was too quiet to evaluate.")

        chroma = self._compute_chroma(samples, sample_rate)
        if chroma.sum() <= 0:
            return self._empty("Could not extract stable pitch information from audio.")

        predicted_chord, confidence, chord_scores = self._match_chord(chroma)
        normalized_target = normalize_chord_name(target_chord or "")
        target_score = chord_scores.get(normalized_target, 0.0)
        matches_target = bool(normalized_target) and target_score >= 0.58 and (
            predicted_chord == normalized_target or confidence - target_score <= 0.06
        )

        top_pitch_indices = np.argsort(chroma)[-4:][::-1]
        pitch_classes = [
            PITCH_CLASSES[index] for index in top_pitch_indices if chroma[index] > 0
        ]
        if matches_target:
            message = f"Audio matches {normalized_target}."
        elif predicted_chord and normalized_target:
            message = f"Audio sounds closer to {predicted_chord} than {normalized_target}."
        elif predicted_chord:
            message = f"Audio sounds closest to {predicted_chord}."
        else:
    ...
    def _empty(self, message: str) -> AudioChordResult:
        return AudioChordResult(False, None, 0.0, False, message, [])

    def _decode_pcm(self, audio_base64: str, audio_format: str, channels: int) -> np.ndarray:
        ...
        if len(audio_bytes) > self.max_audio_bytes:
            raise ValueError("Audio clip is too large. Send a short clip under 2 MB.")
        ...
        if not audio_bytes or len(audio_bytes) % frame_size != 0:
            raise ValueError("PCM byte length does not match its format and channels.")
    ...
    def _compute_chroma(self, samples: np.ndarray, sample_rate: int) -> np.ndarray:
        ...
        mask = (frequencies >= 65.0) & (frequencies <= 1600.0)
        valid_frequencies = frequencies[mask]
        midi_notes = np.rint(69 + 12 * np.log2(valid_frequencies / 440.0)).astype(int)
        frame_chromas = []
        ...
        for start in starts:
            ...
            if float(np.sqrt(np.mean(frame ** 2))) < self.min_rms:
                continue
            power = np.log1p(np.abs(np.fft.rfft(frame * np.hanning(fft_size))) ** 2)[mask]
            ...
            if total > 0:
                frame_chromas.append(frame_chroma / total)

        if not frame_chromas:
            return np.zeros(12, dtype=np.float32)
        chroma = np.median(np.stack(frame_chromas), axis=0).astype(np.float32)
        chroma /= max(float(chroma.sum()), 1e-6)
        return chroma

    def _match_chord(self, chroma: np.ndarray) -> Tuple[Optional[str], float, Dict[str, float]]:
        best_chord = None
        best_score = 0.0
        ...
        for chord, tones in CHORD_TONES.items():
            ...
            score = max(0.0, min(1.0, tone_energy * 0.8 + balance * 0.25 - outside_energy * 0.15))
            scores[chord] = score
            if score > best_score:
                best_chord = chord
                best_score = score
        return best_chord, best_score, scores
